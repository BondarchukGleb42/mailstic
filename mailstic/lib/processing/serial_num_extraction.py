...
from generate_reg_from_pattern import generate_regex
...
def extract_all_serial_numbers(text: str, pattern: Optional[str] = None, max_count_serial_numbers: int = 1) \
        -> List[Union[str, None]]:
    """
...
def extract_serial_number_by_patterns(text, patterns=None):
    ...
    for pattern in patterns:
        res.extend([i for i in extract_all_serial_numbers(text, pattern, max_count_serial_numbers=20) if i is not None])
